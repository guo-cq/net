...
class Dataset(TorchDataset):
    """与 UNet 版本保持一致的数据集定义，单独放在 CNN 目录下。

    - ``root_dir``: 数据根目录
    - ``split_file``: 包含相对路径的 txt 文件
    - ``height``/``width``: 输出尺寸
    - ``transform``/``target_transform``: 输入与真值的变换

    返回的样本结构与原始 ``dataset.Dataset`` 相同，以确保训练/测试脚本
    中的自定义 ``collate`` 逻辑可以复用。
    """
